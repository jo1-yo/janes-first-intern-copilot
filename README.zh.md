...
```bash
# 个人 skill，所有项目都能用（目标文件夹必须叫 jane-first-intern-copilot，和 skill 名字一致）
...
**验证装好了没：** 新开一个对话，输入 `/jane`，命令菜单里应该能补全出 `/jane-first-intern-copilot`；或者直接说 *"I need help finding my first internship."*——如果 Claude 第一步是**先要你的简历再给建议**（而不是直接甩通用 tips），说明 skill 已生效。

## 怎么用

两种触发方式：话题涉及实习、找工作、简历、cover letter、cold email、"投了没人回"时**自动触发**；或者在 Claude Code 里输入 `/jane-first-intern-copilot` **手动调用**。直接正常聊天：
